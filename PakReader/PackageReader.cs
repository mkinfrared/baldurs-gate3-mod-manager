...
public class PackageReader : IDisposable
{
    ...
    private Package ReadPackageV13(FileStream mainStream, BinaryReader reader)
    {
        ...
        {
            string msg =
                $"Unsupported package version {header.Version}; this package layout is only supported for {PackageVersion.V13}";
        ...
        int uncompressedSize = LZ4Codec.Decode64(compressedFileList, 0, compressedFileList.Length, uncompressedList, 0,
            fileBufferSize, true);
        if (uncompressedSize != fileBufferSize)
        {
            string msg =
                $"LZ4 compressor disagrees about the size of file headers; expected {fileBufferSize}, got {uncompressedSize}";
            throw new InvalidDataException(msg);
        }
        ...
        //     {
        //         string msg =
        //             $"Incorrectly compressed solid archive; offsets {firstOffset}/{lastOffset}, bytes {totalSizeOnDisk}";
        //         throw new InvalidDataException(msg);
        //     }
        //
        ...
        //         }
        //
        //         var file = PackagedFileInfo.CreateSolidFromEntry(entry, _streams[entry.ArchivePart], compressedOffset,
        //             decompressedStream);
