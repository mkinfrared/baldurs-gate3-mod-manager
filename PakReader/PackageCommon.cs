...
public class UncompressedPackagedFileStream : Stream
{
    ...
    {
        get { return PackageStream.CanTimeout; }
    }
    ...
    {
        get { return (long)FileInfo.SizeOnDisk; }
    }
...
public class PackagedFileInfo : AbstractFileInfo, IDisposable
{
    ...
    public override Stream MakeStream()
    {
        ...
        if (SizeOnDisk > 0x7fffffff)
        {
            throw new InvalidDataException(
                $"File '{Name}' is over 2GB ({SizeOnDisk} bytes), which is not supported yet!");
        }
        ...
        if (Crc != 0)
        {
            ...
            // {
            //     string msg =
            //         $"CRC check failed on file '{Name}', archive is possibly corrupted. Expected {Crc,8:X}, got {computedCrc,8:X}";
            //     throw new InvalidDataException(msg);
            // }
        }
    ...
    internal static PackagedFileInfo CreateSolidFromEntry(FileEntry13 entry, Stream dataStream, uint solidOffset,
        Stream solidStream)
    {
...
public class FilesystemFileInfo : AbstractFileInfo, IDisposable
{
    ...
    public override Stream MakeStream() =>
        _stream ?? (_stream = File.Open(FilesystemPath, FileMode.Open, FileAccess.Read, FileShare.Read));
